...
def get_capabilites():
    ...
    capabilities.append(_get_capability_dict(service_name='subsetUSGSNEDDEM',
                                             description="Editedd capabilities for subsetUSGSNEDDEM by Prasanna"))
    capabilities.append(_get_capability_dict(service_name='helloworld',
                                             description="Test hello world by Prasanna"))
    capabilities.append(_get_capability_dict(service_name='reclassifyrasterwithlut',
                                             description="A function to reclassify raster"))
    capabilities.append(_get_capability_dict(service_name='delineatewatershedtogetcompleterasterset',
                                             description="A function to delineate watershed and get complete raster set "))
...
def get_service_info(service_name):
    ...
    services_info_dict['subsetUSGSNEDDEM'] = _get_subsetUSGSNEDDEM_info
...
# prasanna addede, from below this point

def _get_subsetUSGSNEDDEM_info():
    service_name = 'subsetUSGSNEDDEM'
    return {service_name: [{'end_point': _get_end_point(service_name), 'http_method': 'GET',
                            'parameters': [
                                           _get_param_dict(name='xmin',
                                                           description='X-coordinate of the left-top corner of the bounding box',
...
def _get_hello_world_info():
    service_name = "helloworld"
    return {service_name: [{'end_point':_get_end_point(service_name),
                        'parameters': [
                            _get_param_dict(name='a',
                                            description='Input float1, trial',
                                            required=True,
                                            type='float'),
                            _get_param_dict(name='b',
                                            description='Input float2, trial',
                                            required=True,
                                            type='float'),
                        ],
                        'http_method': 'GET'
                                        },
                                        _get_json_response_format(data_dict={'test_output': 'url of the txt file created'}
                                                                  )]
          }

def _get_reclassify_raster_with_LUT_info():
    service_name = "reclassifyrasterwithlut"
    return {service_name: [{'end_point':_get_end_point(service_name),
                        'parameters': [
                            _get_param_dict(name='input_raster',
                                            description='Input raster that has to be reclassified',
                                            required=True,
                                            type='string'),
                            _get_param_dict(name='delimiter',
                                            description='delimiter like , or tab etc.',
                                            required=False,
                                            type='string'),
                            _get_param_dict(name='LUT',
                                            description='Look up table. CSV or text file',
                                            required=True,
                                            type='string'),
                        ],
                        'http_method': 'GET'
                                        },
                                        _get_json_response_format(data_dict={'output_raster': 'url of the raster file created'}
                                                                  )]
          }


def _get_delineatewatershedtogetcompleterasterset_info():
    service_name = 'delineatewatershedtogetcompleterasterset'
    return {service_name: [{'end_point': _get_end_point(service_name), 'http_method': 'GET',
                            'parameters': [
                                            _get_param_dict(name='input_DEM_raster',
                                                            description='HydroDS url file path for a user owned raster file that needs to be delineated',
                        ...
                        _get_json_response_format(data_dict={'output_raster': 'url of the output raster file',
                                                             'output_outlet_shapefile': 'url of the output outlet shapefile',
                                                             'output_fill_raster': 'url of the output fill raster',
                                                             'output_slope_raster': 'url of the output slope ',
                                                             'output_flow_direction_raster': 'url of the output flow direction raster',
                                                             'output_contributing_area_raster': 'url of the output contributing area raster',
                                                             'output_accumulated_stream_raster': 'url of the output flow accumulation raster',
                                                             'output_stream_raster': 'url of the output stream raster',
