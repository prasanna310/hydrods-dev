...
args = c('.','Soil_mukey.tif',
       'dth1.tif', 'dth2.tif', 'psif.tif', 'depth.tif',
       'bbl.tif', 'psd.tif', 'rsm.tif', 'ssm.tif', 'ksat_LUT.tif', 'ksat_ssurgo_wtd.tif', 'ksat_ssurgo_min.tif', 'hydrogrp.tif',
       '-111.92798899783645', '-111.3428700880885', '41.6360245560725', '42.16648950172311'  , '182', '165' ,
       'mask.tif') # xmin,xmax,ymin,ymax, ,ncol, nrow
...
mapunit_geom_by_ll_bbox2 <- function (bbox, source = "sda")
{
    ...
    download.file(url = u, destfile = tf.full, quiet = FALSE, method="curl")   #cacheOK=FALSE, method='auto')  #method="curl"
    d <- rgdal::readOGR(dsn = tf.full, layer = file.layer, disambiguateFIDs = TRUE,
...
# assign coordinate system to the feature obtained from SDA
# NOTE, it is very surprising that when I get this res_geometry obj from Linux, there is not CS to it!
# NOTE (cont..), while when you use the exact same fn from windows, there is one. So i had to specify explicitly that this i the cs of the downloaded feature class
crs(res_geometry) <- CRS(" +proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs ")


# reference raster is the input watershed, for which the soil rasters will be made
reference_raster = raster(args[21])
res_geometry_prj = spTransform(res_geometry,proj4string(reference_raster) )


# creating soil_mukey.tif that contains mukeys. This will be reclassified later on based on values for each mukeys.
new_raster = raster(args[21])
ras2 <- rasterize(res_geometry_prj, new_raster, field= as.numeric(res_geometry_prj$mukey) , fun='first' )
writeRaster(ras2, "Soil_mukey.tif", format = "GTiff", overwrite=TRUE)  # 'Soil_mukey should be replace by args[2]
print ('Soil_mukey.tif created succesfully! ')
...
in.statement <- format_SQL_in_statement(unique(res_geometry_prj$mukey)) #unique(x$MUKEY)
...
write.csv(res,'texture_joint_df.csv')
write.csv(res,'soil_data.csv') #this file, soil_data.csv is in the list of output files


print ('Changing NA values to avg')
...
print ("4. Weighted average by component completed successfully" )

# aggregate by map unit
mu1.whc <- ddply(co.whc, 'mukey', mu.mean.whc)

mu.whc = merge(x = mu1.whc, y = res2, by = "mukey",  all = TRUE)
print ("5. Weighted average by map units successful" )


print ('Changing NA values to avg, again')
# give an average value for NA items. For some reason, looks like it does not work if no NA value
# for(i in 1:ncol(mu.whc)){
#   mu.whc[is.na(mu.whc[,i]), i] <- lapply(mu.whc[,i], mean, na.rm = TRUE)  #colMeans(res[,i], na.rm = TRUE) # mean(res[,i], na.rm = TRUE)
# }

#mu.whc = as.numeric(mu.whc)
for(i in 1:ncol(mu.whc)){
  mu.whc[is.na(mu.whc[,i]), i] <- mean(mu.whc[,i], na.rm = TRUE)
}



write.csv(co.whc, 'component_agg_df.csv')
write.csv(mu.whc, 'mapunit_agg_df.csv')
...
for(i in 2:length(soil_data)){
  temp_df = soil_data[,c(1,i)]
  write.table( temp_df, paste(working_dir, '/', names(temp_df)[2],'.csv', sep=""), sep=",", row.names=FALSE, col.names=FALSE)
  system(paste("python3.4 /home/ahmet/ciwater/usu_data_service/pytopkapi_data_service/reclassifyraster.py",'-i', 'Soil_mukey.tif', '-lut',paste(working_dir, '/', names(temp_df)[2],'.csv', sep="") , '-o',
               paste(names(temp_df)[2],'.tif', sep=""), sep=" ")  )
}
