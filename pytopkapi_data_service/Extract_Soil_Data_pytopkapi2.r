...
#args = c('E:/TOPNET/test_folder/test2','Soil_mukey.tif','f.tif', 'k.tif', 'dth1.tif', 'dth2.tif', 'psif.tif', 'sd.tif','tran.tif',
#        'bbl.tif', 'psd.tif', 'rsm.tif', 'ssm.tif', 'ksat.tif', 'ksat_rawls.tif', 'ksat_hz.tif')
...
co.mean.whc <- function(i) {
  ...
  data.frame(ksat=ksat.total,wcthird=wcthirdbar.total,wctfifteendbar=wctfifteendbar.total,  dbthirdbar=dbthirdbar.total,
             sand=sand.total,clay=clay.total,om=om.total,awc=awc.total,awcdepth=awc.depth, thick=1/fval,wt=wt,f=fval,tr=trans, ksat_hz=ksat_hz.total) # return profile water storage and component pct
...
mu.mean.whc <- function(i) {
  ...
  data.frame(depth=thick,ksat=ksat,wcthird=wcthird,wctfifteendbar=wctfifteendbar, dbthirdbar= dbthirdbar,sand=sand,clay=clay,om=om,fval=fvalue,tr=ts, ksat_hz=ksat_hz) # return wt. mean water storage
  #,fval=fvalue*-1
}
...
soil_data=data.frame(mukey=mu.whc$mukey, depth=mu.whc$depth,fvalue=mu.whc$fval,ksat=ko,dth1=dth1,dth2=dth2,c=c1,psif=psif,tr=mu.whc$tr,  BBL=BBL, PSD=PSD, RSM=RSM, SSM=SSM, ksat_rawls=ksat_rawls, ksat_hz=ksat_hz)
...
rat.new <- rat.new[,c("ID", "COUNT","fvalue","ksat","dth1","dth2","psif","depth","tr",           "BBL", "PSD", "RSM", "SSM", "ksat_rawls", "ksat_hz")]
...
q=c(args[3],args[4],args[5],args[6],args[7],args[8],args[9],     args[10],args[11],args[12],args[13],args[14], args[15] )
